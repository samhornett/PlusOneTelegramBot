...
def get_album_info_from_url(shared_url):
    ...
    print(track_id)
    spotify_token = "BQCrVdTlytjqg8DkGKmlJHKhuFknaF2EsSWbX6kKKR7Tb34GEYlP3GP5O1hY23H1aMdmOTUoWNxBoIGCicsxYqf49faAKm7qRg9KF40lHrXhA2NM1ftoedBHyqEKxEfFoc1OBGMk7w"
                ...
                if len(update.message.reply_to_message.entities) > 0:
                    if update.message.reply_to_message.entities[0]["type"] == "url":
                        ...
                        if url.startswith("https://open.spotify.com"):
                            ...
                            context.chat_data["Songs_recommended"]["{} by {}".format(song_name,artist)] += 1
                            context.chat_data["Songs_recommended_total"].setdefault(
                                "{} by {}".format(song_name,artist), 0)
                            context.chat_data["Songs_recommended_total"]["{} by {}".format(song_name,artist)] += number
...
def songs(update, context):
    ...
    try:
        sorted_dict = {k: v for k, v in sorted(context.chat_data["Songs_recommended"].items(), key=lambda item: item[1])}
    except KeyError:
        text = "No scores yet!!"
    else:
        for song,score in sorted_dict.items():
            text += "{} got {} upvotes for a total of {} \n".format(song,score,context.chat_data["Songs_recommended_total"][song] )
